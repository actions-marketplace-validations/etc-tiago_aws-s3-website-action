...
async function invalidateCache(inputParameters: InputParameters) {
  if (
    !inputParameters.cloudFrontDistributionId ||
    !inputParameters.cloudFrontInvalidationPath
  ) {
    throw new Error(
      'Need AWS_CLOUDFRONT_DISTRIBUTION_ID and AWS_CLOUDFRONT_INVALIDATION_PATH to perform the invalidation'
    );
  }
    ...
    cloudfrontClient.createInvalidation(
      {
        ...
        InvalidationBatch: {
          ...
          Paths: {
            Quantity: 1,
            Items: [inputParameters.cloudFrontInvalidationPath]
          }
        }
      },
      (err) => {
